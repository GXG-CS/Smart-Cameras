...
OUTPUT_FILE="../results/image_segmentation_results_$TIMESTAMP.csv"
...
# Loop through configurations and run segment.py
for speed in "${CPU_SPEEDS[@]}"; do
    sudo cpufreq-set -f ${speed}MHz
    
    for cores in "${CORE_CONFIGS[@]}"; do

        # Run the segment.py script and append the output to the results file
        FPS_RESULTS=$(taskset -c ${cores//[-]/,} python3 segment.py --videoPath $VIDEO_PATH)
