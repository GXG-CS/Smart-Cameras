...
def main():
    ...
    results_dir = "results_clustermap"
    ...
    for method in methods:
        ...
        for file in os.listdir(method_dir):
            ...
            # Create a clustermap for correlations
            corr_matrix = df.corr()
            sns.clustermap(corr_matrix, annot=True, cmap='coolwarm', fmt=".2f", figsize=(10, 8))
            plt.title(f"{abbreviations[method]}_{preprocess_name}_{file.split('_')[-1].split('.')[0]}_Clustermap")
            
            clustermap_path = os.path.join(results_dir, f"{abbreviations[method]}_{preprocess_name}_{file.split('_')[-1].split('.')[0]}_clustermap.png")
