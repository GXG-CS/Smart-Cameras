...
folder_path="/home/pi/Smart-Cameras/tf_pose_estimation"
...
for speed in "${CPU_SPEEDS[@]}"; do
  ...
  for cores in "${CORE_CONFIGS[@]}"; do
    ...
    avg_fps=$(taskset -c $core_list python /home/pi/Smart-Cameras/tf_pose_estimation/pose_estimation.py --videoPath /home/pi/Smart-Cameras/output.avi 2>> "$folder_path/error_log.txt" | awk -F',' '{print $1}')

    # Log the average FPS to log.txt
    echo "Average FPS: $avg_fps" >> "$folder_path/log.txt"
    
    # Construct the CSV line
    csv_line="${sdram_freq_list[$counter]},$cores,$speed,$avg_fps"
